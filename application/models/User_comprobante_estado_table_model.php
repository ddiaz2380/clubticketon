...
class User_comprobante_estado_table_model extends CI_Model
{
    public $v_fields=array('user_com_estado');
	...
	function getList($table, $pagination=array()) {
        ...
        $this->db->order_by("user_com_es_id","desc");
        $query = $this->db->get();
    ...
    function getRow($table, $id) {
        $this->db->select('*');
        $query = $this->db->get_where($table, array('user_com_es_id' => $id));
        $data = $query->result();
        return $data[0];
    }
    ...
    {
        $this->db->where("user_com_es_id",$id);
