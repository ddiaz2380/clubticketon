...
class Municipio_table_model extends CI_Model
{
    public $v_fields=array('municipio_nombre');
	...
	function getList($table, $pagination=array()) {
        ...
        $this->db->order_by("municipio_id","desc");
        $query = $this->db->get();
    ...
    function getRow($table, $id) {
        $this->db->select('*');
        $query = $this->db->get_where($table, array('municipio_id' => $id));
        $data = $query->result();
        return $data[0];
    }
