...
class User_comprobante_table_model extends CI_Model
{
    public $v_fields=array('user_table.username', 'com_fecha_subida', 'com_fecha_compra', 'com_numero', 'com_cuit', 'municipio_table.municipio_nombre', 'image_link', 'user_comprobante_estado_table.user_com_estado', 'com_estado_date', 'com_estado_obs', 'com_estado_opeid', 'product_cart_id', 'cart_id');
	...
	function getList($table, $pagination=array()) {
        ...
        $this->db->select("$table.*  , user_table.username as user_id  , municipio_table.municipio_nombre as municipio_id  , user_comprobante_estado_table.user_com_estado as com_estado_id ");
        $this->db->from($table);
         $this->db->join("user_table", "user_comprobante_table.user_id = user_table.user_id", "left");  $this->db->join("municipio_table", "user_comprobante_table.municipio_id = municipio_table.municipio_id", "left");  $this->db->join("user_comprobante_estado_table", "user_comprobante_table.com_estado_id = user_comprobante_estado_table.user_com_es_id", "left");     
        $this->db->order_by("user_comprobante_id","desc");
        $query = $this->db->get();
    ...
    function getRow($table, $id) {
        $this->db->select('*');
        $query = $this->db->get_where($table, array('user_comprobante_id' => $id));
        $data = $query->result();
        return $data[0];
    }
    ...
    {
        $this->db->where("user_comprobante_id",$id);
