...
class Account extends CI_Controller {
    ...
    **/
    ...
    /**
        Signing In
        Checking user type to redirect different Panel de administracion
    **/
    public function loggingIn() {
        ...
        if ($result == "user") {
            $redirect_to   = 'shop';
            $flash_type    = 'success';
            $flash_content = '<div class="alert alert-primary mt-4"role="alert">Has iniciado sesión con éxito in as <span href="#" class="alert-link">'.$username.'</span>. Happy browsing.</div>';
        } else if ($result == "admin") {
            $redirect_to   = 'Admin';
            $flash_type    = 'success';
            $flash_content = '<div class="alert alert-primary mt-4" role="alert">Has iniciado sesión con éxito in as <span href="#" class="alert-link">'.$username.'</span>. You are the Admin.</div>';
        } else if ($result == "ban") {
            $redirect_to   = 'shop';
            $flash_type    = 'success';
    ...
    **/
    public function logout()
    {
		...
		$this->session->sess_destroy(); //Add Vilte
        redirect('shop');
    }
}
